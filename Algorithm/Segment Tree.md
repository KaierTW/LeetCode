...
→ tree[1] = min(2, 6) = 2
```
你問得很好！這裡是 Segment Tree 的建構過程中最關鍵也最容易搞混的地方。
讓我來**逐步視覺化與拆解**你看到的這一段：
...
我們要建一棵 Segment Tree，每個節點記錄一段區間的最小值。
...
* 左葉子：`build(4, 0, 0)`，只有 A\[0]
* 右葉子：`build(5, 1, 1)`，只有 A\[1]
...
```cpp
tree[2] = min(tree[4], tree[5]) = min(5, 2) = 2
```
...
* 左葉子：`build(6, 2, 2)` → A\[2] = 8
* 右葉子：`build(7, 3, 3)` → A\[3] = 6
...
```cpp
tree[1] = min(tree[2], tree[3]) = min(2, 6) = 2
```
...
你可以把這棵樹看成「Segment Tree 儲存在 vector 中的樣子」，像這樣：
...
如果你想更清楚「節點編號怎麼安排」或「怎麼用公式存左右子節點」，我也可以幫你補上～

你可以再問：例如怎麼用 `query(1, 0, 3, 1, 2)` 查詢 A\[1]\~A\[2] 的最小值？我也可以示範一次完整流程。
